...
class AffineLayerSuite extends AnyFunSpec with Matchers:
  ...
  describe("AffineLayer"):
    ...
    it("identity"):
      val affineLayer = AffineLayer(AffineLayer.Params.identity(Axis[A] -> 5, VType[Float32]))
    ...
    it("bias"):
      ...
      val affineLayer = AffineLayer(AffineLayer.Params.identity(Axis[A] -> 5, VType[Float32]).copy(bias = bias))
    ...
    it("weight"):
      ...
      val affineLayer = AffineLayer(AffineLayer.Params.identity(Axis[A] -> 5, VType[Float32]).copy(weight = weight))
