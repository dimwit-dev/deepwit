...
/** Samples a 2D convolution kernel from the Xavier/Glorot uniform distribution.
  ...
  */
private[cnn] def xavierUniformKernel[S1: Λ, S2: Λ, InChannel: Λ, OutChannel: Λ, V: IsFloating](s1Extent: AxisExtent[S1], s2Extent: AxisExtent[S2], channelExtent: AxisExtent[InChannel], outChannelExtent: AxisExtent[OutChannel], vtype: VType[V], key: Key): Tensor[(S1, S2, InChannel, OutChannel), V] =
